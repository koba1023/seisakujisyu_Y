...
// 実行（itemsテーブルからデータ取得）
$sql_items = "SELECT * FROM nagataku_items0 WHERE town LIKE '%{$search_town}%'";
$rec_items = $dbh->prepare($sql_items);
$rec_items->execute();
$rec_list_items = $rec_items->fetchAll(PDO::FETCH_ASSOC);

// 実行（items2テーブルからデータ取得）
$sql_items2 = "SELECT * FROM nagataku_items1 WHERE town LIKE '%{$search_town}%'";
$rec_items2 = $dbh->prepare($sql_items2);
$rec_items2->execute();
$rec_list_items2 = $rec_items2->fetchAll(PDO::FETCH_ASSOC);
} else {
// 「検索」ボタン押下してないとき（itemsテーブルからデータ取得）
$sql_items = 'SELECT * FROM nagataku_items0 WHERE 1';
$rec_items = $dbh->prepare($sql_items);
$rec_items->execute();
$rec_list_items = $rec_items->fetchAll(PDO::FETCH_ASSOC);

// （items2テーブルからデータ取得）
$sql_items2 = 'SELECT * FROM nagataku_items1 WHERE 1';
$rec_items2 = $dbh->prepare($sql_items2);
$rec_items2->execute();
$rec_list_items2 = $rec_items2->fetchAll(PDO::FETCH_ASSOC);
}
